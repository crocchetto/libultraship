...
Context::Context(std::string name, std::string shortName)
    : Component(std::move(name)), mShortName(std::move(shortName)), mLastTickTime(), mHasTicked(false) {
}
...
double Context::Tick() {
    ...
    auto tickableComponents = mTickableComponents.Get();
    for (const auto& tickableComponent : *tickableComponents) {
        tickableComponent->Run(durationSinceLastTick);
    }
